import React from 'react'

function Solutions() {
  return (
    <div>
      <section class="text-gray-400 bg-gray-900 body-font overflow-hidden">
      <div class="container px-5 py-24 mx-auto">
          <div class="flex flex-col text-center w-full mb-20">
            <h1 class="sm:text-4xl text-3xl font-medium title-font mb-2 font-bold text-white">DIGITAL BADGES, <br/> DIGITAL CERTIFICATES <br/> AND AWARDS</h1>
            <p class="lg:w-2/3 mx-auto leading-relaxed text-base">Connect with Best Employers</p>
          </div>
          <div class="flex flex-wrap -m-4 mx-5 py-5">
            <div class="p-4 xl:w-1/2 md:w-1/2 w-full">
              <div class="h-full p-6 rounded-lg border-2 border-gray-700 flex flex-col relative overflow-hidden">
              <h1 class="text-5xl text-white pb-4 mb-4 leading-none">PRASASTI</h1>
              <h5 class=" text-white pb-4 mb-4 border-b border-gray-800 leading-none">  FOR COLLEGES AND UNIVERSITIES</h5>
                <p class="flex items-center text-gray-400 mb-2">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-indigo-800 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Issue Digital Bagdes and Certificates to Students
                </p>
                <p class="flex items-center text-gray-400 mb-2">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-indigo-800 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Boost Student Engagement
                </p>
                <p class="flex items-center text-gray-400 mb-2">
                <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-800 text-gray-500 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Promote Your College
                </p>
                <p class="flex items-center text-gray-400 mb-6">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-800 text-gray-500 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Allows Job-seeking Students to share their Digital Awards
                </p>
                <button class="flex items-center justify-center mt-auto text-white bg-indigo-800 border-0 py-2 px-4 w-full focus:outline-none hover:bg-gray-700 rounded text-center">Button
                  <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-auto" viewBox="0 0 24 24">
                    <path d="M5 12h14M12 5l7 7-7 7"></path>
                  </svg>
                </button>
              </div>
            </div>
            <div class="p-4 xl:w-1/2 md:w-1/2 w-full">
              <div class="h-full p-6 rounded-lg border-2 border-gray-700 flex flex-col relative overflow-hidden">
              <h1 class="text-5xl text-white pb-4 mb-4 leading-none">PRASASTI</h1>
              <h5 class=" text-white pb-4 mb-4 border-b border-gray-800 leading-none">FOR SCHOOLS</h5>
                <p class="flex items-center text-gray-400 mb-2">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-indigo-800 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Builds Students Digital Profile from School Age
                </p>
                <p class="flex items-center text-gray-400 mb-2">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-indigo-800 rounded-full flex-shrink-0">
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Enables Aspirations in Students
                </p>
                <p class="flex items-center text-gray-400 mb-2">
                <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-indigo-800 rounded-full flex-shrink-0">
                  <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Promote your School
                </p>
                <p class="flex items-center text-gray-400 mb-6">
                  <span class="w-4 h-4 mr-2 inline-flex items-center justify-center bg-gray-800 text-gray-500 rounded-full flex-shrink-0">
                  <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" class="w-3 h-3" viewBox="0 0 24 24">
                      <path d="M20 6L9 17l-5-5"></path>
                    </svg>
                  </span>Provides Global Recognition to Students
                </p>
                <button class="flex items-center justify-center mt-auto text-white bg-indigo-800 border-0 py-2 px-4 w-full focus:outline-none hover:bg-gray-700 rounded text-center">Button
                  <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-auto" viewBox="0 0 24 24">
                    <path d="M5 12h14M12 5l7 7-7 7"></path>
                  </svg>
                </button>
              </div>
            </div>
          </div>
        </div>
      </section>

    {/* Section 2 */}

    <div className='box p-5 m-5'>
    <div className='box-container'>
      <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Blockchain based Solution that Stores Your Achievements</h1>
      </div>
      <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Showcase your Skills in LinkedIn and other Social Media Platforms</h1>
      </div>
      <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Provides LifeTime Value</h1>
      </div>
    </div>
    <div className='box-container'>
    <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Builds Your Professional Digital Reputation</h1>
      </div>
      <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Win Rewards for your Achievements.</h1>
      </div>
      <div className='container p-5'>
          <div class="w-20 h-20 inline-flex items-center justify-center rounded-full bg-indigo-500 text-indigo-500 mb-5 flex-shrink-0">
              <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M8.27668 6.39118C8.52657 5.869 8.6665 5.28415 8.6665 4.66663C8.6665 2.45749 6.87564 0.666626 4.6665 0.666626C2.45736 0.666626 0.666504 2.45749 0.666504 4.66663C0.666504 6.87577 2.45736 8.66663 4.6665 8.66663C5.28403 8.66663 5.86888 8.52669 6.39106 8.2768L8.4377 10.3234C7.73967 11.3774 7.33317 12.6412 7.33317 14C7.33317 15.3587 7.73967 16.6226 8.4377 17.6765L6.69056 19.4236C6.47026 19.3647 6.23872 19.3333 5.99984 19.3333C4.52708 19.3333 3.33317 20.5272 3.33317 22C3.33317 23.4727 4.52708 24.6666 5.99984 24.6666C7.4726 24.6666 8.6665 23.4727 8.6665 22C8.6665 21.7611 8.63509 21.5295 8.57618 21.3092L10.3233 19.5621C11.3772 20.2601 12.6411 20.6666 13.9998 20.6666C15.3586 20.6666 16.6224 20.2601 17.6764 19.5621L19.723 21.6087C19.4731 22.1309 19.3332 22.7158 19.3332 23.3333C19.3332 25.5424 21.124 27.3333 23.3332 27.3333C25.5423 27.3333 27.3332 25.5424 27.3332 23.3333C27.3332 21.1242 25.5423 19.3333 23.3332 19.3333C22.7156 19.3333 22.1308 19.4732 21.6086 19.7231L19.562 17.6765C20.26 16.6226 20.6665 15.3587 20.6665 14C20.6665 12.6412 20.26 11.3774 19.562 10.3234L21.3091 8.5763C21.5294 8.63522 21.761 8.66663 21.9998 8.66663C23.4726 8.66663 24.6665 7.47272 24.6665 5.99996C24.6665 4.5272 23.4726 3.33329 21.9998 3.33329C20.5271 3.33329 19.3332 4.5272 19.3332 5.99996C19.3332 6.23884 19.3646 6.47038 19.4235 6.69068L17.6764 8.43782C16.6224 7.7398 15.3586 7.33329 13.9998 7.33329C12.6411 7.33329 11.3772 7.7398 10.3233 8.43782L8.27668 6.39118Z" fill="white"/>
              </svg>
          </div>
        <h1 className='text-2xl'>Provides Global Recognition</h1>
      </div>

    </div>
    </div>


    {/* Last Secton */}

    <section class="text-gray-600 body-font mx-5 px-5">
      <div class="container mx-auto flex px-5 py-24 md:flex-row flex-col items-center">
        <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center p-5">
          <h1 class="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-900">CERTIFICATION PROGRAM <br/> PROVIDERS</h1>
          <p class="mb-8 leading-relaxed">Easy to Generate Certificates
Students can easily share their certificates on Linkedin and other Social Media Platforms
Provides Wider Recognition in the market for the Certified Students
Drives Visibility and Demand for Your Certification Programs</p>
          <div class="flex w-full md:justify-start justify-center items-end">
          <button class=" text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg text-start">Start now <i className='fa fa-arrow-right'></i></button>
          </div>
        </div>
        <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
          <img class="object-cover object-center rounded" alt="hero" src="solution1.jpg" />
        </div>
      </div>
    </section>

    <section class="text-gray-600 body-font mx-5 px-5">
      <div class="container mx-auto flex px-5 py-24 md:flex-row flex-col items-center">
      <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
          <img class="object-cover object-center rounded" alt="hero" src="solution1.jpg" />
        </div>
        <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center p-5">
          <h1 class="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-900">HACKATHON ORGANISERS</h1>
          <p class="mb-8 leading-relaxed">Quickly Generate Certificates to Hackathon Winners
Certificates are easily verifiable by Employers
Participates can share the awards on Linkedin and other Social Media Platforms
Provides Global Recognition to Students
Helps Job Seeking Participants to Get Jobs Easily </p>
          <div class="flex w-full md:justify-start justify-center items-end">
          <button class=" text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg text-start">Start now <i className='fa fa-arrow-right'></i></button>
          </div>
        </div>
      </div>
    </section>

    <section class="text-gray-600 body-font mx-5 px-5">
      <div class="container mx-auto flex px-5 py-24 md:flex-row flex-col items-center">
        <div class="lg:flex-grow md:w-1/2 lg:pl-24 md:pl-16 flex flex-col md:items-start md:text-left items-center p-5">
          <h1 class="title-font sm:text-4xl text-3xl mb-4 font-medium text-gray-900">EMPLOYEE SERVICE CERTIFICATE</h1>
          <p class="mb-8 leading-relaxed">Easily Generate Employee Service and Experience Letter
Cost Effective Solution
Saves Time as no need to Re-Issue Lost Certificates.
          </p>
          <div class="flex w-full md:justify-start justify-center items-center">
            <button class=" text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-lg text-start">Start now <i className='fa fa-arrow-right'></i></button>
          </div>
        </div>
        <div class="lg:max-w-lg lg:w-full md:w-1/2 w-5/6 mb-10 md:mb-0">
          <img class="object-cover object-center rounded" alt="hero" src="solution1.jpg" />
        </div>
      </div>
    </section>

    {/* Reviews */}

    <div className=' text-center p-5 container-height light-bg -mb-5'>
      <div className='pt-5 mt-5'>
        <h1 className='p-heading text-center'>TALENT ACQUISITION FOR EMPLOYERS</h1>
      </div>

      <div>
      <section class="text-gray-600 body-font">
          <div class=" px-5 py-24 mx-auto">
            <div class="flex flex-wrap -m-4">
              <div class="lg:w-1/3 lg:mb-0 mb-6 p-4">
                <div className='flex items-center justify-center stars py-5'>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                </div>
                <div class="h-full text-center">
                  <h2 class="text-gray-900 font-medium title-font tracking-wider text-l">Gain Access to Best Talent</h2>
                </div>
              </div>
              <div class="lg:w-1/3 lg:mb-0 mb-6 p-4">
              <div className='flex items-center justify-center stars py-5'>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                </div>
                <div class="h-full text-center">
                  <h2 class="text-gray-900 font-medium title-font tracking-wider text-l">Source and Screen Candidates with Verified Skills</h2>
                </div>
              </div>
              <div class="lg:w-1/3 lg:mb-0 mb-6 p-4">
              <div className='flex items-center justify-center stars py-5'>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                <i className='fa fa-star gold text-xl'></i>
                </div>
                <div class="h-full text-center">
                  <h2 class="text-gray-900 font-medium title-font tracking-wider text-l">Easily Verify the Candidate Skills using Blockchain Verification Link</h2>
                </div>
              </div>
            </div>
          </div>
        </section>
      </div>

      </div>

    </div>  
  )
}

export default Solutions